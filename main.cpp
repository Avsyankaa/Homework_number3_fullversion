...
void sort_matrix_snake (  int**&Matrix, int &m, int &n) {
    int *Matrix_string = new int [m*n];
    int k=0;
    for (int i=0; i<m; i++) {
        for (int j=0; j<n; j++) {
    ...
    for (int i=0; i< (m*n)-1; i++) {
        for (int j=0; j< (m*n)- i-1; j++) {
            if (Matrix_string[j]> Matrix_string[j+1])
              swap (Matrix_string[j], Matrix_string[j+1]);
        }
    }
    for (int j=0; j<n; j++) {
        if ((j+1)%2==1) {
...
void sort_matrix_escargot (  int**&Matrix, int &m, int &n) {
    int *Matrix_string = new int [m*n];
    int k=0;
    for (int i=0; i<m; i++) {
        for (int j=0; j<n; j++) {
    ...
    while (curr_kvadr != (kvadr + 1)) {
        for (int i = (curr_kvadr - 1); i < (n - curr_kvadr + 1); i++) {
            Matrix[curr_kvadr - 1][i] = Matrix_string[k];
            k++;
        }
        for (int i = curr_kvadr; i < (m - curr_kvadr + 1); i++) {
            Matrix[i][n - curr_kvadr] = Matrix_string[k];
            k++;
        }
        for (int i = (n - curr_kvadr - 1); i > (curr_kvadr - 2); i--) {
            Matrix[m - curr_kvadr][i] = Matrix_string[k];
            k++;
        }
        for (int i = (m - curr_kvadr - 1); i > (curr_kvadr - 1); i--) {
...
void sort_matrix_aunt (  int**&Matrix, int &m, int &n) {
    int *Matrix_string = new int [m*n];
    int k=0;
    for (int i=0; i<m; i++) {
        for (int j=0; j<n; j++) {
...
void sort_the_matrix (int **&Matrix, int &m, int &n) {
    if (Matrix == nullptr) cout << "Matrix is empty!!!" << endl;
    else {
        char choise_str;
        cout<< "Select the sort order:" << endl;
        cout << "s: snake;" << endl;
        cout << "e: escargot;" <<endl;
        cout << "a: aunt;" << endl;
        cin >> choise_str;
        int choise;
        if (choise_str == 's') choise =1;
        if (choise_str == 'e') choise =2;
        if (choise_str == 'a') choise =3;
        switch (choise) {
        case 1:
            sort_matrix_snake (Matrix, m, n);
            break;
        case 2:
            sort_matrix_escargot (Matrix, m, n);
            break;
        case 3:
            sort_matrix_aunt (Matrix, m, n);
            break;
        }
    }
}
